...
export async function createRevision(data: Omit<NewRevision, 'id' | 'createdAt' | 'updatedAt'>): Promise<Revision> {
  return window.db.createRevision(data);
}

export async function updateRevision(id: string, data: Partial<Omit<NewRevision, 'id' | 'createdAt'>>): Promise<Revision> {
