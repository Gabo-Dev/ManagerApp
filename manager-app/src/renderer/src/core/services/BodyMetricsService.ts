...
export class BodyMetricsService {
    public static calcular(input: BodyMetricsInput): CalculosComposicion {
        ...
        const suma4Pliegues = pliegues.tricipital + pliegues.escapular + pliegues.abdominal + pliegues.iliaco;
        const logSuma = Math.log10(suma4Pliegues || 1);
 
        let densidad = 0;
        if (tipoActividad === 'suave') {
            densidad = sexo === 'H' ? 1.1620 - (0.0630 * logSuma) : 1.1339 - (0.0645 * logSuma);
        } else {
