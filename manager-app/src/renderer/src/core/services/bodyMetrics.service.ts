...
export const BodyMetricsService = {
  calculate(inputs: MetricInputs): CalculatedMetrics {
    const {
      pesoAyunasKg, estaturaM, edad, sexo, tipoActividad,
      tricipitalMm, escapularMm, abdominalMm, cIliacaMm,
      circunferenciaBrazoCm, circunferenciaMusloCm, circunferenciaPantorrillaCm
    } = inputs;
    ...
    if (suma4Pliegues > 0) {
      if (tipoActividad === 'suave') {
        densidadCorporal = sexo === 'H' 
          ? 1.1620 - (0.0630 * Math.log10(suma4Pliegues))
    ...
    if (densidadCorporal > 0) {
      let grasa = (495 / densidadCorporal) - 450;
      metrics.porcentajeGrasa = parseFloat(Math.max(3, Math.min(50, grasa)).toFixed(2));
      
      if (pesoAyunasKg > 0) {
        metrics.pesoGrasoKg = parseFloat(((pesoAyunasKg * (metrics.porcentajeGrasa as number)) / 100).toFixed(2));
        metrics.mlgKg = parseFloat((pesoAyunasKg - (metrics.pesoGrasoKg as number)).toFixed(2));
      }
    }

    const hasMMEInputs = pesoAyunasKg > 0 && estaturaM > 0 && edad > 0 &&
      circunferenciaBrazoCm > 0 && circunferenciaMusloCm > 0 && circunferenciaPantorrillaCm > 0;
    ...
    if (hasMMEInputs) {
      ...
      ) + (2.4 * sexFactor) - (0.048 * edad) + 7.8;
      
      metrics.porcentajeMme = parseFloat(mme.toFixed(2));

      if (typeof metrics.porcentajeGrasa === 'number') {
        metrics.porcentajeOtros = parseFloat(Math.max(0, 100 - metrics.porcentajeGrasa - (metrics.porcentajeMme as number)).toFixed(2));
      }
    }
